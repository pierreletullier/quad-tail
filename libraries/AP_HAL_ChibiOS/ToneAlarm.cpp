...
bool ToneAlarm::init()
{
    // TODO: RCOutput::init() starts all of the PWM drivers like this:
//    pwmStart(pwm_group_list[i].pwm_drv, &pwm_group_list[i].pwm_cfg);

    tune_num = 0;                    //play startup tune

    // register 1kHz timer callback
//    hal.scheduler->register_timer_process(FUNCTOR_BIND_MEMBER(&ToneAlarm::_timer_tick, void));
...
//        dprintf(period_fd,"%u",1000000000/cur_note);
        // hardwire timer number and channel to TIM2_1
        // need to specify buzzer pwmgroup/channel in the config
