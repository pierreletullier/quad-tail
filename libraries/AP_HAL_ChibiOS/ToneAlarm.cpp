...
struct ToneAlarm::pwm_group ToneAlarm::pwm_group_list[] = { HAL_PWM_GROUPS };
...
{
    // start PWM driver
    pwm_group_list[BUZZER_TIMER-1].pwm_cfg.period = 1000;
    pwm_group_list[BUZZER_TIMER-1].pwm_cfg.channels[BUZZER_CHANNEL-1].mode = PWM_OUTPUT_ACTIVE_HIGH;
    pwmStart(pwm_group_list[BUZZER_TIMER-1].pwm_drv, &pwm_group_list[BUZZER_TIMER-1].pwm_cfg);
...
void ToneAlarm::stop()
{
    pwmDisableChannel(pwm_group_list[BUZZER_TIMER-1].pwm_drv, BUZZER_CHANNEL-1);
...
bool ToneAlarm::play()
{
    ...
    if(cur_note != 0){
        ...
        pwmChangePeriod(pwm_group_list[BUZZER_TIMER-1].pwm_drv,
                        pwm_group_list[BUZZER_TIMER-1].pwm_cfg.frequency/cur_note);

        pwmEnableChannel(pwm_group_list[BUZZER_TIMER-1].pwm_drv, BUZZER_CHANNEL-1,
                         (pwm_group_list[BUZZER_TIMER-1].pwm_cfg.frequency/2)/cur_note);
